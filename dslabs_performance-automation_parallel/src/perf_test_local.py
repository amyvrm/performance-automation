...
class PerformanceScenario(PerfCommon):
    def __init__(self, machine_info, ver, access_key, secret_key, stats, graph, path_json, nexus_uname, nexus_pwd):
        ...
        self.dsm = DsmPolicy(ver, self.policy_name, nexus_uname, nexus_pwd, machine, path_json)
        # self.dsm.upload_basic_policy()
        # self.dsm.download_package(package_url)
        ...
        sip, s_priv_ip = "35.183.106.245", "172.31.30.143"
        ...
        cip, c_priv_ip = "15.223.114.143", "172.31.25.151"
    ...
    def test_scenario(self, sip, spwd, cip, cpwd, s_priv_ip, c_priv_ip, suser, cuser, scenario_name):
        ...
        time.sleep(120)
        wo_filter_all_stats = self.run_band_test(sip, suser, spwd, cip, cuser, cpwd, s_priv_ip, c_priv_ip, scenario_name)
        ...
        time.sleep(120)
        good_rule_all_stats = self.run_band_test(sip, suser, spwd, cip, cuser, cpwd, s_priv_ip, c_priv_ip, scenario_name)
        ...
        time.sleep(120)
        with_all_server_rule_all_stats = self.run_band_test(sip, suser, spwd, cip, cuser, cpwd, s_priv_ip, c_priv_ip, scenario_name)
        ...
        print("{0}{0}\n- With All Server side rule: {1}\n- Average Bandwidth: {2} MBps\n{0}{0}\n".format(self.header,
                                                                                    with_all_server_rule, all_rule_avg))
        ...
        df = pd.DataFrame([wo_filter_stats, w_filter_stats, good_rule, with_all_server_rule], index=self.col, columns=self.title)
        # Create Html
...
if __name__ == '__main__':
    ...
    parser.add_argument('--package_url', type=str, help="Need package url to apply in dsm")
    ...
    url = "https://dsnexus.trendmicro.com:8443/nexus/repository/dslabs/Testing/perf/8/manifest.json"
