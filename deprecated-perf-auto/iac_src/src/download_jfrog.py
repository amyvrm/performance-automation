...
def get_pkg(url, path, jfrog_token):
    print("# {} Downloading.... #".format(url))
    name, ext = os.path.splitext(url.rsplit("/", 1)[-1])
    pkg = requests.get(url, auth=BearerAuth(jfrog_token))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--url', type=str, help="JFrog Package URL")
    parser.add_argument('--path', type=str, help="JFrog Package URL")
    parser.add_argument('--jfrog_token', type=str, help="jfrog_token")
    args = parser.parse_args()

    get_pkg(args.url, args.path, args.jfrog_token)
