resource "aws_instance" "dsa_windows_machine_2" {
	...
	tags = {
			Name = "${local.dsa_windows_machine_2_name}"
...
//			"Automation" = "Performance Automation"
	...
	connection {
			type = "winrm"
			host     = "${aws_instance.dsa_windows_machine_2.public_ip}"
			timeout  = "${var.conn_timeout}"
			user     = "${var.conn_user}"
			password =  "${rsadecrypt(aws_instance.dsa_windows_machine_2.password_data,file("${var.auth_file_path}/${var.terraform_user}.pem"))}"
			insecure = "true"	
		}
...
output "dsa-windows-id-2" {
	value = "${aws_instance.dsa_windows_machine_2.id}"
}
output "dsa-public-ip-2" {
	value = "${aws_instance.dsa_windows_machine_2.public_ip}"
}
output "dsa-user-2" {
	value = "${var.conn_user}"
}
