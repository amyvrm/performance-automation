...
FILENAME="/SolarisAgentDeploymentScript.sh"
cat > $PATH$FILENAME<<EOF1
#!/bin/bash

ACTIVATIONURL='dsm://$DSM_MACHINE_IP:4120/'
...
SOURCEURL='https://$DSM_MACHINE_IP:4119/';
AGENTSEGMENT='software/agent/';
CURL='curl ';
CURL_OPTIONS=' --insecure --silent --tlsv1.2 -o ';
CURL_MAJOR_MIN=7;
CURL_MINOR_MIN=34;
P5P_PACKAGE='/tmp/agent.p5p.gz';
PKG_PACKAGE='/tmp/agent.pkg.gz';
platform='Solaris_';
version=`uname -r`;
arch='';
isP5P=1;
...
if type curl > /dev/null 2>&1; then
    ...
    if [ -z "${curl_major}" ] || [ -z "${curl_minor}" ]; then
        ...
        if [ ${isP5P} -eq 1 ]; then
            downloadAction=$CURL$SOURCEURL$AGENTSEGMENT$platform$version$update$arch$CURL_OPTIONS$P5P_PACKAGE;
            rm -rf "${P5P_PACKAGE}";
        else
            downloadAction=$CURL$SOURCEURL$AGENTSEGMENT$platform$version$update$arch$CURL_OPTIONS$PKG_PACKAGE;
            rm -rf "${PKG_PACKAGE}";
        fi
        ...
        if [ ${rc} -eq 0 ]; then
            ...
            # /opt/ds_agent/dsa_control -a dsm://172.31.24.206:4120/
        else
