resource "aws_instance" "dsa_debian10_machine" {
	
	ami = "${var.dsa_debian10_ami}"
	...
	tags = {
		Name = "${local.dsa_debian10_machine_name}"
...
output "dsa-debian10-id" {
  value = "${aws_instance.dsa_debian10_machine.id}"
}
