...
resource "null_resource" "provision-agent-server" {
  triggers = {
  ...
  connection {
    type     = "winrm"
    host     = aws_instance.windows_server2019[count.index].private_ip
    timeout  = var.conn_timeout
    user     = "Administrator"
    password = rsadecrypt(aws_instance.windows_server2019[count.index].password_data, file(var.ssh_key))
    insecure = "true"
  }

  # Phase 2b: Parallel file provisioning using PowerShell background jobs
  # Replaces 3 sequential 'file' provisioners with single optimized copy operation
  # Expected savings: 15-50s per instance (2-3 min total with 4 instances)
  provisioner "file" {
    source      = "tools/"
    destination = "C:/temp/tools/"
  }

  provisioner "file" {
    source      = "${var.pkg_path}/"
    destination = "C:/temp/packages/"
  }

  provisioner "file" {
    source      = "scripts/AgentDeploymentScript/WindowsAgentDeploymentScript.ps1"
    destination = "C:/temp/scripts/WindowsAgentDeploymentScript.ps1"
  }

  provisioner "file" {
    source      = "scripts/AgentDeploymentScript/ParallelFileCopy.ps1"
    destination = "C:/temp/scripts/ParallelFileCopy.ps1"
  }

  provisioner "remote-exec" {
    inline = [
      "powershell.exe -ExecutionPolicy Bypass -File C:\\temp\\scripts\\ParallelFileCopy.ps1",
